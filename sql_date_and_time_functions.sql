...
В новом поле month для каждого фильма необходимо отобразить месяц выхода основываясь на значение поля release_date и преобразовать значение поля month в нижний регистр.
...
Выберите поля title , rating и release_year
Значения поля title преобразовать в верхний регистр.
В новом поле day для каждого фильма необходимо отобразить день выхода основываясь на значение поля release_date и преобразовать значение поля day в верхний регистр.
Результат должен быть отсортирован по полю release_year в порядке убывания.*/
...
Значения поля title преобразовать в верхний регистр.
В новом поле world_publish_date  должна хранится новая дата для всех фильмов. К году из поля release_date прибавьте 30 дней и выведите результат.*/
